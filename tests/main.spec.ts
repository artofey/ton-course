...
describe("main.fc contract tests", () => {
  ...
  it("successfully deposits funds", async () => {
    ...
    const depositeMessageResult = await myContract.sendDeposit(
      senderWallet.getSender(),
      toNano("5")
    );

    expect(depositeMessageResult.transactions).toHaveTransaction({
      from: senderWallet.address,
      to: myContract.address,
      success: true,
    });
  ...
  it("should return deposit funds as no command is sent", async () => {
    ...
    const depositeMessageResult = await myContract.sendNoCodeDeposit(
      senderWallet.getSender(),
      toNano("5")
    );
